generate_categoricalT_training <- function(seed, size, uncertaintyW, w1, w2){
  ...
  A1 <- node("A", distr = "rcat.b1",  probs = c(0.25, 0.25, 0.25, 0.25))
...
meanFun <- function(Hvec,A)
{
  ...
  {
    if(A[i] == 0) return(0)
    else if (A[i]==1) return(5*(simuFun(H1[i])*simuFun(H2[i])-2))
    else if (A[i]==2) return(-5*(simuFun(H1[i])*simuFun(H2[i])-2))
    else if (A[i]==3) return(1*(simuFun(H1[i])*simuFun(1-H2[i])-2))
