...
source('GCT-Utils-Simulation-Categorical.R')
...
suppressMessages(suppressWarnings(
  {
    trainingData <- generate_categoricalT_training(seed = seed+repId, size = 500, 
                                uncertaintyW=uncertaintyW, w1, w2)
    trainingDataProcessed <- process_nonbinary_treatment(trainingData) 
    testingData <- generate_categoricalT_training(seed = seed+repId+1, size = 500,
                               uncertaintyW=uncertaintyW, w1, w2)
                                                                          ...
                                                                          trainingDataProcessed)
{
    ...
    get_multiple_treatment_Effects <- function(cohortId)
    {
      ...
      for(zlevelsId in 1:length(Zrange))
      {
        ...
        #sampleTr <- sample[which(sample$A==1 & sample$Z==currZlevel), "Y1"]
        sampleTr <- sample[which(sample$treatment==1 & sample$Z==currZlevel), "Y1"]
        #sampleTe <- sample[which(sample$A==0 & sample$Z==currZlevel), "Y1"]
